...
import { delAdminDialog, getAdminDialogs, postAdminDialog, putAdminDialog } from '@/request/adminApi';
...
function MessagePage() {
    ...
    const columns: ProColumns<DialogInfo>[] = [
        ...
        },
        {
            ...
            width: 120,
            dataIndex: 'delay',
            render: (_, data) => <Tag>{`随机0-${data.delay}毫秒`}</Tag>
        },
        {
            title: '状态值',
            width: 100,
            dataIndex: 'status',
            render: (_, data) => <Tag color={data.status ? 'green' : 'red'}>{data.status ? '正常' : '隐藏'}</Tag>
        },
        {
            title: '创建时间',
            width: 200,
